...
from tools.drive_flat_sync import (
    ...
    DEFAULT_PROJECT_LONGEVITY_FOLDER_ID,
)
...
def test_resolve_drive_root_id_project_longevity_fallback(monkeypatch):
    monkeypatch.delenv("DRIVE_FOLDER_ID", raising=False)
    monkeypatch.delenv("DRIVE_ROOT_FOLDER_ID", raising=False)
    monkeypatch.setenv("PROJECT_LONGEVITY_FOLDER_ID", "project")
    assert resolve_drive_root_id() == "project"


def test_resolve_drive_root_id_default_project_longevity_folder(monkeypatch):
    monkeypatch.delenv("DRIVE_FOLDER_ID", raising=False)
    monkeypatch.delenv("DRIVE_ROOT_FOLDER_ID", raising=False)
    monkeypatch.delenv("PROJECT_LONGEVITY_FOLDER_ID", raising=False)
    assert resolve_drive_root_id() == DEFAULT_PROJECT_LONGEVITY_FOLDER_ID
