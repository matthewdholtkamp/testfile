# Draft Status

- **Title:** Blood-Brain Barrier Failure as a biomarker-guided manuscript path in traumatic brain injury
- **Candidate:** Blood-Brain Barrier Failure biomarker panel
- **Pack key:** most_informative_biomarker_panel_packet_blood_brain_barrier_failure
- **Generated status:** ready_for_metadata_only
- **Ready for metadata only:** yes
- **Gate state:** ready to build phase 8 pack
- **Primary journal:** Journal of Neurotrauma
- **Recommended article type:** review
- **Scientific strength:** 2 / 4
- **Journal fit:** 4 / 4
- **Draft readiness:** 4 / 4
- **Current top blocker:** Downstream lane is still seeded/provisional, so this transition should remain bounded.
- **Expected readouts:** Barrier leakage, Ocln / cldn5 / tjp1 restoration, Cerebral blood flow, Downstream inflammatory spillover
- **Next test:** Run an acute-to-subacute BBB repair packet around OCLN with Evans blue or gadolinium leakage, CLDN5/OCLN/TJP1 restoration, and a downstream inflammatory spillover panel.

## Linked Evidence Chain

- Phase 1: Blood Brain Barrier Failure
- Phase 2: BBB Permeability Increase To Peripheral Immune Infiltration
- Phase 3: Microglial Chronic Activation, Neurovascular Uncoupling
- Phase 4: Blood Brain Barrier Failure
- Phase 5: Acute Mild Biomarker Imaging Bridge, Acute Severe Vascular Dominant, Acute Blast Vascular Inflammatory Mixed, Subacute Repetitive Inflammatory Dominant

## Remaining Work

- Operator metadata items: 12
- Content or rigor gaps: 0

## Phase Artifacts In Scope

- phase1: Blood-Brain Barrier Failure (longitudinally_supported)
- phase2: BBB permeability increase -> peripheral immune infiltration (supported)
- phase3: Microglial Chronic Activation (supported)
- phase3: Neurovascular Uncoupling (provisional)
- phase4: Blood-Brain Barrier Failure (bounded)
- phase5: Acute Mild Biomarker / Imaging Bridge (bounded)
- phase5: Acute Severe Vascular-Dominant (usable)
- phase5: Acute Blast Vascular / Inflammatory Mixed (bounded)
- phase5: Subacute Repetitive Inflammatory-Dominant (usable)
